...
async def apply_watermark(video_path: Path, logo_path: Path, out_path: Path, *, video_width: int) -> bool:
    ...
    watermark_px = max(24, round(video_width * _LOGO_WIDTH_FRACTION))
    filter_complex = (
        f"[1:v]scale={watermark_px}:-1,format=rgba,colorchannelmixer=aa={_OPACITY}[wm];"
        f"[0:v][wm]overlay=main_w-overlay_w-{_MARGIN_PX}:main_h-overlay_h-{_MARGIN_PX}[out]"
    )
    cmd = [
        "ffmpeg", "-y",
        "-i", str(video_path),
